...
const AUTH_TOKEN = 'eyJhbGciOiJIUzUxMiJ9.eyJ1c2VySWQiOiIxODBkNTVjNy00ODEwNGIzYi1iMDI0LTRiOTExMjM4ZDE5YSIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTc1MjgyMTE0MSwiZXhwIjoxNzUyOTA3NTQxfQ.LTvNa5P_TQKfmDC3oe8PSYP571ZrbMTBrfFtpJEiX5PtGLOMo8z2prcwwHTSMis6aTzgDejsVwkbH_9A8zCjKw';
...
const HeaderDataTable = ({ search, roleFilter, setSearch, setRoleFilter, roleOptions }) => (
    <div className="flex align-items-center justify-content-between gap-2 flex-wrap">
        <div>
            <span className="text-xl font-bold mr-3">Employees</span> {/* Changed from Users to Employees */}
            <Dropdown
...
const EmployeeAdminPage = () => {
    ...
    const [roleFilter, setRoleFilter] = useState("employee"); // Default to filter by 'employee'

    const toast = useRef(null);

    // Only allow "employee" role for creation/editing in this dashboard
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            // Filter users to only show 'employee' roles by default
            setUsers(result.data.filter(user => user.role === 'employee'));
            showToast('success', 'Success', result.message || 'Employees retrieved successfully!');
        } catch (err) {
            console.error("Failed to fetch employees:", err);
            showToast('error', 'Error', `Failed to retrieve employees: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const openNewUserDialog = () => {
        ...
        setFormData({
            ...
            password: '',
            full_name: '',
            role: 'employee', // Force 'employee' role for new users in this dashboard
            is_active: true,
        });
        setFormErrors({});
    ...
    const openEditUserDialog = (user) => {
        ...
        setFormData({
            ...
            role: user.role || 'employee', // Maintain existing role, or default to 'employee'
    ...
    const saveUser = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                role: 'employee', // Always set role to 'employee' from this dashboard
            ...
            if (!response.ok) {
                ...
                showToast('success', 'Success', result.message || `Employee ${currentUser ? 'updated' : 'created'} successfully!`);
                fetchUsers();
                hideFormDialog();
            }
        } catch (err) {
            console.error(`Failed to ${currentUser ? 'update' : 'create'} employee:`, err);
            showToast('error', 'Error', `Failed to ${currentUser ? 'update' : 'create'} employee: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmDeleteSelected = () => {
        if (selectedUsers.length === 0) {
            showToast('warn', 'No Selection', 'Please select employees to delete.');
            return;
        }
        confirmDialog({
            message: `Are you sure you want to delete ${selectedUsers.length} selected employee(s)?`,
    ...
    const deleteSelectedUsers = async () => {
        ...
        const deletePromises = selectedUsers.map(async (user) => {
            try {
                ...
                console.error(`Error deleting employee ${user.username}:`, err);
                errorCount++;
        ...
        if (successCount > 0) {
            showToast('success', 'Success', `${successCount} employee(s) deleted successfully.`);
        }
        if (errorCount > 0) {
            showToast('error', 'Error', `${errorCount} employee(s) failed to delete.`);
        }
    ...
    const confirmDeleteUser = (user) => {
        confirmDialog({
            message: `Are you sure you want to delete employee ${user.username}?`,
            header: 'Confirm Deletion',
    ...
    const deleteUser = async (id) => {
        ...
        try {
            ...
            showToast('success', 'Success', result.message || 'Employee deleted successfully!');
            fetchUsers();
        } catch (err) {
            console.error("Failed to delete employee:", err);
            showToast('error', 'Error', `Failed to delete employee: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredUsers = users.filter((user) => {
        // Always filter by 'employee' role as this dashboard is specific to employees
    ...
    return (
        <>
            ...
            <div className="card p-6 rounded-lg shadow-xl bg-white m-4">
                <h3 className="text-3xl font-bold text-gray-800 mb-6">Employee Administration</h3> {/* Changed title */}
                ...
                <Dialog
                    header={currentUser ? "Edit Employee" : "Add New Employee"} 
                ...
                >
                    ...
                    {/* Role dropdown specifically for 'employee' dashboard */}
                    <div className="p-field mb-4">
                        ...
                        <Dropdown
                            ...
                            name="role"
                            value={formData.role}
                            options={roleOptions} // Only "employee" is selectable
                            onChange={onFormInputChange}
                            placeholder="Select a Role"
                            disabled // Disable selection as it's fixed to 'employee'
                        />
                    </div>
