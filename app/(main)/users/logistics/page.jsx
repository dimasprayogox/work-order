...
const HeaderDataTable = ({ search, roleFilter, setSearch, setRoleFilter, roleOptions }) => (
    <div className="flex align-items-center justify-content-between gap-2 flex-wrap">
        <div>
            <span className="text-xl font-bold mr-3">Logistics Personnel</span> {/* Changed title */}
            <Dropdown
...
const LogisticsAdminPage = () => {
    ...
    const [roleFilter, setRoleFilter] = useState("logistics"); // Default to filter by 'logistics'

    const toast = useRef(null);

    // Only allow "logistics" role for creation/editing in this dashboard
    const roleOptions = ["logistics"];
    ...
    const [formData, setFormData] = useState({
        ...
        password: '',
        full_name: '',
        role: 'logistics', // Default role for new logistics personnel
        is_active: true,
    });
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            // Filter users to only show 'logistics' roles by default
            setUsers(result.data.filter(user => user.role === 'logistics'));
            showToast('success', 'Success', result.message || 'Logistics personnel retrieved successfully!');
        } catch (err) {
            console.error("Failed to fetch logistics personnel:", err);
            showToast('error', 'Error', `Failed to retrieve logistics personnel: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const openNewUserDialog = () => {
        ...
        setFormData({
            ...
            password: '',
            full_name: '',
            role: 'logistics', // Force 'logistics' role for new users in this dashboard
            is_active: true,
        });
        setFormErrors({});
    ...
    const openEditUserDialog = (user) => {
        ...
        setFormData({
            ...
            role: user.role || 'logistics', // Maintain existing role, or default to 'logistics'
    ...
    const validateForm = () => {
        ...
        if (!currentUser && !formData.password.trim()) {
            newErrors.password = 'Password is required for new logistics personnel.';
    ...
    const saveUser = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                role: 'logistics', // Always set role to 'logistics' from this dashboard
            ...
            if (!response.ok) {
                ...
                showToast('success', 'Success', result.message || `Logistics personnel ${currentUser ? 'updated' : 'created'} successfully!`);
                fetchUsers();
                hideFormDialog();
            }
        } catch (err) {
            console.error(`Failed to ${currentUser ? 'update' : 'create'} logistics personnel:`, err);
            showToast('error', 'Error', `Failed to ${currentUser ? 'update' : 'create'} logistics personnel: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmDeleteSelected = () => {
        if (selectedUsers.length === 0) {
            showToast('warn', 'No Selection', 'Please select logistics personnel to delete.');
            return;
        }
        confirmDialog({
            message: `Are you sure you want to delete ${selectedUsers.length} selected logistics personnel?`,
    ...
    const deleteSelectedUsers = async () => {
        ...
        const deletePromises = selectedUsers.map(async (user) => {
            try {
                ...
                console.error(`Error deleting logistics personnel ${user.username}:`, err);
                errorCount++;
        ...
        if (successCount > 0) {
            showToast('success', 'Success', `${successCount} logistics personnel deleted successfully.`);
        }
        if (errorCount > 0) {
            showToast('error', 'Error', `${errorCount} logistics personnel failed to delete.`);
        }
    ...
    const confirmDeleteUser = (user) => {
        confirmDialog({
            message: `Are you sure you want to delete logistics personnel ${user.username}?`,
            header: 'Confirm Deletion',
    ...
    const deleteUser = async (id) => {
        ...
        try {
            ...
            showToast('success', 'Success', result.message || 'Logistics personnel deleted successfully!');
            fetchUsers();
        } catch (err) {
            console.error("Failed to delete logistics personnel:", err);
            showToast('error', 'Error', `Failed to delete logistics personnel: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredUsers = users.filter((user) => {
        // Always filter by 'logistics' role as this dashboard is specific to logistics personnel
        ...
        return (
            <div className="flex gap-2">
                <Button
                    ...
                    tooltip="Edit Logistics Personnel"
                ...
                <Button
                    ...
                    tooltip="Delete Logistics Personnel"
                    tooltipOptions={{ position: 'bottom' }}
                />
            </div>
        );
    ...
    return (
        <>
            ...
            <div className="card p-6 rounded-lg shadow-xl bg-white m-4">
                <h3 className="text-3xl font-bold text-gray-800 mb-6">Logistics Administration</h3> {/* Changed title */}

                <div className="flex flex-wrap gap-3 mb-6">
                    <Button
                        size="small"
                        label="New Logistics Personnel"
                        icon="pi pi-plus"
                        severity="success"
                ...
                {/* Add/Edit Logistics Personnel Dialog */}
                <Dialog
                    header={currentUser ? "Edit Logistics Personnel" : "Add New Logistics Personnel"}
                ...
                >
                    ...
                    {/* Role dropdown specifically for 'logistics' dashboard */}
                    <div className="p-field mb-4">
                        ...
                        <Dropdown
                            ...
                            name="role"
                            value={formData.role}
                            options={roleOptions} // Only "logistics" is selectable
                            onChange={onFormInputChange}
                            placeholder="Select a Role"
                            disabled // Disable selection as it's fixed to 'logistics'
                        />
                    </div>
...
export default LogisticsAdminPage;