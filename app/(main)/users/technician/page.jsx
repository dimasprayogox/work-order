...
const HeaderDataTable = ({ search, roleFilter, setSearch, setRoleFilter, roleOptions }) => (
    <div className="flex align-items-center justify-content-between gap-2 flex-wrap">
        <div>
            <span className="text-xl font-bold mr-3">Technicians</span> {/* Changed title */}
            <Dropdown
...
const TechnicianAdminPage = () => { {/* Renamed the component */}
    ...
    const [roleFilter, setRoleFilter] = useState("technician"); // Default to filter by 'technician'

    const toast = useRef(null); 

    // Only allow "technician" role for creation/editing in this dashboard
    const roleOptions = ["technician"]; 
    ...
    const [formData, setFormData] = useState({
        ...
        password: '',
        full_name: '',
        role: 'technician', // Default role for new technicians
        is_active: true,
    });
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            // Filter users to only show 'technician' roles by default
            setUsers(result.data.filter(user => user.role === 'technician')); 
            showToast('success', 'Success', result.message || 'Technicians retrieved successfully!');
        } catch (err) {
            console.error("Failed to fetch technicians:", err);
            showToast('error', 'Error', `Failed to retrieve technicians: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const openNewUserDialog = () => {
        ...
        setFormData({
            ...
            password: '',
            full_name: '',
            role: 'technician', // Force 'technician' role for new users in this dashboard
            is_active: true,
        });
        setFormErrors({});
    ...
    const openEditUserDialog = (user) => {
        ...
        setFormData({
            ...
            role: user.role || 'technician', // Maintain existing role, or default to 'technician'
    ...
    const saveUser = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                role: 'technician', // Always set role to 'technician' from this dashboard
            ...
            if (!response.ok) {
                ...
                showToast('success', 'Success', result.message || `Technician ${currentUser ? 'updated' : 'created'} successfully!`);
                fetchUsers(); 
                hideFormDialog();
            }
        } catch (err) {
            console.error(`Failed to ${currentUser ? 'update' : 'create'} technician:`, err);
            showToast('error', 'Error', `Failed to ${currentUser ? 'update' : 'create'} technician: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmDeleteSelected = () => {
        if (selectedUsers.length === 0) {
            showToast('warn', 'No Selection', 'Please select technicians to delete.');
            return;
        }
        confirmDialog({
            message: `Are you sure you want to delete ${selectedUsers.length} selected technician(s)?`,
    ...
    const deleteSelectedUsers = async () => {
        ...
        const deletePromises = selectedUsers.map(async (user) => {
            try {
                ...
                console.error(`Error deleting technician ${user.username}:`, err);
                errorCount++;
        ...
        if (successCount > 0) {
            showToast('success', 'Success', `${successCount} technician(s) deleted successfully.`);
        }
        if (errorCount > 0) {
            showToast('error', 'Error', `${errorCount} technician(s) failed to delete.`);
        }
    ...
    const confirmDeleteUser = (user) => {
        confirmDialog({
            message: `Are you sure you want to delete technician ${user.username}?`,
            header: 'Confirm Deletion',
    ...
    const deleteUser = async (id) => {
        ...
        try {
            ...
            showToast('success', 'Success', result.message || 'Technician deleted successfully!');
            fetchUsers(); 
        } catch (err) {
            console.error("Failed to delete technician:", err);
            showToast('error', 'Error', `Failed to delete technician: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredUsers = users.filter((user) => {
        // Always filter by 'technician' role as this dashboard is specific to technicians
        const matchesRole = user.role === 'technician'; 
        const matchesSearch =
            search === "" ||
    ...
    return (
        <>
            ...
            <div className="card p-6 rounded-lg shadow-xl bg-white m-4">
                <h3 className="text-3xl font-bold text-gray-800 mb-6">Technician Administration</h3> {/* Changed title */}
                ...
                <Dialog
                    header={currentUser ? "Edit Technician" : "Add New Technician"}                    visible={isFormDialogVisible}
                    style={{ width: "50vw" }}
                ...
                >
                    ...
                    {/* Role dropdown specifically for 'technician' dashboard */}
                    <div className="p-field mb-4">
                        ...
                        <Dropdown
                            ...
                            name="role"
                            value={formData.role}
                            options={roleOptions} // Only "technician" is selectable
                            onChange={onFormInputChange}
                            placeholder="Select a Role"
                            disabled // Disable selection as it's fixed to 'technician'
                        />
                    </div>
