...
const PartFormDialog = ({ visible, onHide, part, fetchParts, showToast, assets = [], machines = [] }) => {
    ...
    const validateForm = () => {
        ...
        return form.name && form.part_number && form.location && selectionType && ((selectionType === "asset" && form.asset_id) || (selectionType === "machine" && form.machine_id));
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const endpoint = part ? `/api/logistics/parts/${part.id}` : "/api/logistics/parts";
            ...
            if (res.ok && data.success) {
                showToast("success", "Sukses", data.message || `Part ${part ? "updated" : "created"} successfully`);
                fetchParts();
                onHide();
            } else {
    ...
    return (
        <Dialog header={part ? "Edit Part" : "Add New Part"} visible={visible} style={{ width: "40rem" }} breakpoints={{ "960px": "75vw", "641px": "90vw" }} onHide={onHide} modal className="p-fluid">
            <div className="grid">
                {/* Part Name */}
                ...
                <div className="field col-12 mb-4">
                    ...
                    <InputTextarea id="description" value={form.description} onChange={(e) => handleChange("description", e.target.value)} placeholder="Enter description (optional)" rows={3} />
                </div>
                ...
                <div className="field col-12 md:col-6 mb-4">
                    ...
                    <Dropdown
                        ...
                        showClear
                    />
                    {submitted && !selectionType && <small className="p-error">Please select Asset or Machine</small>}
                </div>
                ...
                {selectionType === "asset" && (
                    <div className="field col-12 md:col-6 mb-4">
                        ...
                        <Dropdown value={form.asset_id} options={assets.map((a) => ({ label: a.name, value: a.id }))} onChange={(e) => handleSelectAsset(e.value)} placeholder="Select Asset" showClear />
                        {submitted && selectionType === "asset" && !form.asset_id && <small className="p-error">Please select an Asset</small>}
                    </div>
                )}
                ...
                {selectionType === "machine" && (
                    <div className="field col-12 md:col-6 mb-4">
                        ...
                        <Dropdown value={form.machine_id} options={machines.map((m) => ({ label: m.name, value: m.id }))} onChange={(e) => handleSelectMachine(e.value)} placeholder="Select Machine" showClear />
                        {submitted && selectionType === "machine" && !form.machine_id && <small className="p-error">Please select a Machine</small>}
                    </div>
                )}
            </div>
