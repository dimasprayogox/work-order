// app/(main)/admin/divisions/page.jsx
"use client";
...
const DivisionPage = () => {
    ...
    const [fileName, setFileName] = useState("Divisions");
    ...
    const [columnOptions] = useState([
        { field: 'name', header: 'Division Name', visible: true },
            ...
            const rowData = columnOptions
                ...
                .map(col => {
                    if (col.field === 'created_at' || col.field === 'updated_at') {
                        return formatDate(division[col.field]);
                    } else {
                        return division[col.field] || '';
                    }
                });
    ...
    const exportPdf = (config = null) => {
        ...
        doc.text('Divisions Report', currentConfig.marginLeft, currentConfig.marginTop);
    ...
    const handleDeleteSelected = () => {
        if (selectedDivisions.length === 0) {
            showToast("warn", "Warning", "Tidak ada divisi yang dipilih");
            return;
        }
        ...
        <div className="p-4">
            ...
            <div className="card">
                <div className="flex justify-content-between items-start mb-4">
                    <div>
                        <h3 className="text-2xl font-semibold">Division Management</h3>
                        <p className="text-sm text-gray-500">Manage division data in the system.</p>
                    </div>
                </div>
                ...
                <div className="flex flex-row gap-2 mb-4">
                    ...
                    <Divider layout="vertical" />
                    <Button size="small" label={`Delete${selectedDivisions.length > 0 ? ` (${selectedDivisions.length})` : ""}`} icon="pi pi-trash" severity="danger" outlined onClick={handleDeleteSelected} disabled={selectedDivisions.length === 0} />
                    <Divider layout="vertical" />
                    <Button size="small" label="Refresh" icon="pi pi-refresh" outlined onClick={fetchDivisions} disabled={loading} />
                </div>
