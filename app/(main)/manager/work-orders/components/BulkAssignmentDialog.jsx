// app/(main)/work-orders/components/BulkAssignmentDialog.jsx
"use client";
...
const BulkAssignmentDialog = ({
    ...
    useEffect(() => {
        if (workOrders && workOrders.length > 0) {
            const unassignedWorkOrders = workOrders.filter((wo) => !wo.assigned_to_id);

            const initialAssignments = unassignedWorkOrders.map((wo) => ({
                work_order_id: wo.id,
                work_order: wo,
                assigned_to_id: "",
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/manager/work-orders/bulk-assign", {
                method: "POST",
