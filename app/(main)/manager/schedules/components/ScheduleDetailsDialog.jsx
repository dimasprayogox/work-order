...
export default function ScheduleDetailsDialog({ visible, onHide, schedule }) {
    // Guard: if no schedule provided, render nothing to avoid runtime errors
                    ...
                    <div className="grid">
                        <div className="col-12 md:col-6">
                            <div className="field mb-4">
                                ...
                                <Tag
                                    value={<span className="flex align-items-center gap-1"><i className={getFrequencyDetails(schedule.frequency).icon}></i> {getFrequencyDetails(schedule.frequency).label}</span>}
                                    severity={getFrequencyDetails(schedule.frequency).severity}
                                    className="font-medium text-base"
                                />
                                <p className="text-sm text-gray-600 mt-1">{getFrequencyDetails(schedule.frequency).description}</p>
                            </div>
                        </div>
                        ...
                        <div className="col-12 md:col-6">
                            <div className="field mb-4">
                                ...
                                <Tag
                                    value={<span className="flex align-items-center gap-1"><i className={getPriorityDetails(schedule.priority).icon}></i> {getPriorityDetails(schedule.priority).label}</span>}
                                    severity={getPriorityDetails(schedule.priority).severity}
                                    className="font-medium text-base"
                                />
                                <p className="text-sm text-gray-600 mt-1">{getPriorityDetails(schedule.priority).description}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="field mb-4">
                        ...
                        <div className="p-3 border-1 border-gray-300 border-round bg-gray-50">
                            <div className="flex align-items-center gap-2 mb-2">
                                <i className={`${getDueDateStatus(schedule.next_due_date).icon} text-${getDueDateStatus(schedule.next_due_date).severity === 'danger' ? 'red' : getDueDateStatus(schedule.next_due_date).severity === 'warning' ? 'yellow' : getDueDateStatus(schedule.next_due_date).severity === 'info' ? 'blue' : 'green'}-500`}></i>
                                <span className="font-medium">{formatDate(schedule.next_due_date)}</span>
                            </div>
                            <Tag
                                value={getDueDateStatus(schedule.next_due_date).message}
                                severity={getDueDateStatus(schedule.next_due_date).severity}
                                className="text-sm"
                            />
                        </div>
                    </div>

                    {calculateNextDueDates(schedule.next_due_date, schedule.frequency).length > 0 && (
                        <>
                            <Divider />
                            <div className="field mb-4">
                                ...
                                <div className="p-3 border-1 border-gray-300 border-round bg-blue-50">
                                    <p className="text-sm text-gray-600 mb-3">Based on {getFrequencyDetails(schedule.frequency).label.toLowerCase()} frequency:</p>
                                    <div className="grid">
                                        {calculateNextDueDates(schedule.next_due_date, schedule.frequency).map((date, index) => (
                    ...
                    <div className="field mb-3">
                        ...
                        <div className="p-3 border-1 border-blue-200 border-round bg-blue-50">
                            <div className="text-sm text-blue-800">
                                <p className="mb-2">
                                    <strong>Maintenance Type:</strong> {getFrequencyDetails(schedule.frequency).label}
                                </p>
                                <p className="mb-2">
                                    <strong>Priority Level:</strong> {getPriorityDetails(schedule.priority).label}
                                </p>
                                <p className="mb-0">
                                    <strong>Status:</strong> {getDueDateStatus(schedule.next_due_date).message}
                                </p>
                            </div>
                        </div>
                    </div>
