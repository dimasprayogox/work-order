...
export default function EditScheduleDialog({ visible, onHide, schedule, showToast, onScheduleUpdated }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await response.json();
            if (!response.ok) {
                throw new Error(result.message || "Gagal memperbarui jadwal perawatan.");
            }
    ...
    const dialogFooter = (
        <div className="flex justify-content-end gap-2">
            ...
            <Button label="Simpan Perubahan" icon="pi pi-check" onClick={handleSubmit} loading={loading} />
        </div>
    );

    return (
        <Dialog
            header={`Edit Jadwal: ${schedule?.title || ""}`}
            visible={visible}
