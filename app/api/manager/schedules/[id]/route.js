...
export const DELETE = async (request, { params }) => {
    ...
    try {
        const response = await Axios.delete(API_ENDPOINTS.MANAGER_SCHEDULE_BY_ID(id), {
            headers: { Authorization: `Bearer ${token}` }
        });
        return NextResponse.json(response.data);
    } catch (err) {
        ...
        console.error(`[API MANAGER SCHEDULES DELETE PROXY - ID: ${id}]`, err);
        return NextResponse.json({ message: "Gagal menghapus jadwal perawatan." }, { status: 500 });
    }
};