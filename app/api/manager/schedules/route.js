// api/MANAGER/schedules/route.js
...
export const POST = async (request) => {
    ...
    try {
        ...
        if (body.ids && Array.isArray(body.ids)) {
            const response = await Axios.post(API_ENDPOINTS.MANAGER_MAINTENANCE_SCHEDULES_DELETE_MANY, body, { headers: { Authorization: `Bearer ${token}` } });
            return NextResponse.json(response.data, { status: 200 });
        }

        // Handle regular create request
        const response = await Axios.post(API_ENDPOINTS.MANAGER_MAINTENANCE_SCHEDULES, body, { headers: { Authorization: `Bearer ${token}` } });
        ...
        return NextResponse.json({ message: "Gagal memproses permintaan jadwal perawatan." }, { status: 500 });
    }
};