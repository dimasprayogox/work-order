...
export const GET = async (request, { params }) => {
    ...
    if (!id) {
        return NextResponse.json({ message: "Part Request ID is required" }, { status: 400 });
    }
    try {
        ...
        console.error("[API TECHNICIAN PART REQUEST GET]", err);
...
export const PATCH = async (request, { params }) => {
    ...
    try {
        ...
        const response = await Axios.patch(API_ENDPOINTS.TECHNICIAN_PART_REQUEST_BY_ID(id), body, {
            headers: { Authorization: `Bearer ${token}` }
        });
        return NextResponse.json(response.data);
    } catch (err) {
        ...
        return NextResponse.json({ message: "Gagal memperbarui part request." }, { status: 500 });
    }
};
...
export const DELETE = async (request, { params }) => {
    ...
    try {
        const response = await Axios.delete(API_ENDPOINTS.TECHNICIAN_PART_REQUEST_BY_ID(id), {
            headers: { Authorization: `Bearer ${token}` }
        });
       return NextResponse.json(response.data);
    } catch (err) {
